 ...
 */
public class ONOSLLDP_ofdpv2 extends LLDP {

    private static final Logger log = getLogger(ONOSLLDP_ofdpv2.class);
    ...
    public ONOSLLDP_ofdpv2(byte... subtype) {
        super();
        for (byte st : subtype) {
    ...
    // In OFDPv2, the LLDP payload is not important.
    private ONOSLLDP_ofdpv2(LLDP lldp) {
     ...
     */
    //this seems ok
    public static ONOSLLDP_ofdpv2 parseONOSLLDP(Ethernet eth)
    {
        //0x88cc, 0x8942
        if (eth.getEtherType() == Ethernet.TYPE_LLDP || eth.getEtherType() == Ethernet.TYPE_BSN)
        {
            //If eth is LLDP or BDDP, then get the payload of Ethernet.
            ONOSLLDP_ofdpv2 onosLldp = new ONOSLLDP_ofdpv2((LLDP) eth.getPayload());
            if (ONOSLLDP_ofdpv2.DEFAULT_NAME.equals(onosLldp.getNameString()))
            {
                return onosLldp;
            }
        }
     ...
     */
    //this seems ok
    public static ONOSLLDP_ofdpv2 parseLLDP(Ethernet eth) {
        if (eth.getEtherType() == Ethernet.TYPE_LLDP || eth.getEtherType() == Ethernet.TYPE_BSN)
        {
            return new ONOSLLDP_ofdpv2((LLDP) eth.getPayload());
        }
     ...
     */
   @Deprecated
    public static ONOSLLDP_ofdpv2 onosLLDP(String deviceId, ChassisId chassisId, int portNum) {
        ONOSLLDP_ofdpv2 probe = new ONOSLLDP_ofdpv2(NAME_SUBTYPE, DEVICE_SUBTYPE);
    ...
    public static ONOSLLDP_ofdpv2 onosSecureLLDP(String deviceId, ChassisId chassisId, String fingerprint, int portNum, String secret) {
        ...
        if (secret == null) {
            ...
            probe = new ONOSLLDP_ofdpv2(NAME_SUBTYPE, DEVICE_SUBTYPE, TIMESTAMP_SUBTYPE, SIG_SUBTYPE);
        }
        probe.setPortId(portNum);
        probe.setDevice(deviceId);
        probe.setChassisId(chassisId);
     ...
     */
    @Deprecated
    public static ONOSLLDP_ofdpv2 onosLLDP(String deviceId, ChassisId chassisId, int portNum, String portDesc) {
        ONOSLLDP_ofdpv2 probe = onosLLDP(deviceId, chassisId, portNum);
    ...
    private static void addPortDesc(ONOSLLDP_ofdpv2 probe, String portDesc) {
    ...
    public static boolean verify(ONOSLLDP_ofdpv2 probe, String secret, long maxDelay) {
        if (secret == null) {
            return true;
        }
